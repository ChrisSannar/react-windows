...
const Bar: React.FC<BarProps> = (props) => {
  ...
  const [rearrangingWindowIndex, setRearrangingWindowIndex] = useState<number>(
    -1,
  )

  // Styles for each window item
  const [windowItemStyles, setWindowItemStyles] = useState<
    React.CSSProperties[]
  >([])

  // When we click on the window item, where our mouse is in relation to that window item
  const [windowItemMousePositions, setWindowItemMousePositions] = useState<
    number[]
  >([])
  ...
  const slideWindowItemToX = (windowItemIndex: number, x: number): void => {
    const newWindowItemStyles = [...windowItemStyles].map((items) => ({
      ...items,
    })) // This map thing because typescript is difficult sometimes (read-only)

    // Get the current item boundaries
    const rect: DOMRect | undefined = windowItems.current[
      windowItemIndex
    ]?.getBoundingClientRect()
    newWindowItemStyles[windowItemIndex].position = 'absolute'

    // Prevent it from overlapping the hr
    const hrBoundary: number =
      (hr.current?.getBoundingClientRect().right ?? 0) + 5
    if (rect) {
      if (x - windowItemMousePositions[windowItemIndex] > hrBoundary) {
        newWindowItemStyles[windowItemIndex].left =
          x - windowItemMousePositions[windowItemIndex] + 'px'
      } else {
  ...
  const handleWindowItemMouseDown = (
    index: number,
  ...
  const moveWindowItemToPosition = (index: number): void => {
    ...
    const newWindowItemStyles: React.CSSProperties[] = [
      ...windowItemStyles,
    ].map((items) => ({ ...items }))
    ...
    const ourItemLeft: number =
      windowItems.current[index]?.getBoundingClientRect().left ?? 0
    if (ourItemLeft) {
      ...
      for (let i: number = windowItems.current.length - 1; i >= 0; i--) {
        ...
        const currentItemLeft: number =
          windowItems.current[i]?.getBoundingClientRect().left ?? 0
    ...
    // If the index didn't change, just toggle the minimize. Otherwise, move it to the new position
    if (index === newIndex) {
